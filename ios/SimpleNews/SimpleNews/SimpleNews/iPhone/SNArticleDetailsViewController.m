//
//  SNArticleDetailsViewController.m
//  SimpleNews
//
...
@interface SNArticleDetailsViewController () <MBLResourceObserverProtocol>
@property(nonatomic, strong) MBLAsyncResource *imageResource;
@end

@implementation SNArticleDetailsViewController
...
//		cardBG = @"defaultCardTimeline_noLikes.png";
	...
	offset += 8;
	if (_vo.type_id > 4) {
		_videoPlayerView = [[SNArticleVideoPlayerView alloc] initWithFrame:CGRectMake(15.0, offset, 290.0, 202.0) articleVO:_vo];
...
-(void)_goArticleSource {
	...
	SNWebPageViewController *webPageViewController = [[SNWebPageViewController alloc] initWithURL:[NSURL URLWithString:_vo.article_url] title:_vo.topicTitle];
	[self.navigationController pushViewController:webPageViewController animated:YES];
}

-(void)_goTwitterProfile {
	SNWebPageViewController *webPageViewController = [[SNWebPageViewController alloc] initWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"https://twitter.com/#!/%@", _vo.twitterHandle]] title:[NSString stringWithFormat:@"@%@", _vo.twitterHandle]];
	[self.navigationController pushViewController:webPageViewController animated:YES];
}

-(void)_goComments {
	[self.navigationController pushViewController:[[SNArticleCommentsViewController alloc] initWithArticleVO:_vo] animated:YES];
}

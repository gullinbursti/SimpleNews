...
- (void)loadView {
	...
	SNHeaderView *headerView;
	
	if (_isFinder) {
		_tableView.contentInset = UIEdgeInsetsMake(12.0, 0.0f, 12.0f, 0.0f);
		
		headerView = [[SNHeaderView alloc] initWithTitle:@"Invite Friends"];
		...
		headerView = [[SNHeaderView alloc] initWithTitle:@"My Friends"];
		[self.view addSubview:headerView];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	SNTwitterFriendViewCell *cell = [tableView dequeueReusableCellWithIdentifier:nil];
		...
		if (cell == nil) {
			...
			if ([_friends count] > 1) {
				if (indexPath.section == 0 && indexPath.row == 0) {
					cell = [[SNTwitterFriendViewCell alloc] initAsHeader];
				
				} else if (indexPath.section == [tableView numberOfSections] - 1 && indexPath.row == [tableView numberOfRowsInSection:indexPath.section] - 1) {
					cell = [[SNTwitterFriendViewCell alloc] initAsFooter];
				
				} else {
					cell = [[SNTwitterFriendViewCell alloc] initAsMiddle];
				}
			
			} else
				cell = [[SNTwitterFriendViewCell alloc] initAsSolo];
		}
...
//		}];
		...
		[UIView animateWithDuration:0.25 animations:^(void) {
			((SNTwitterFriendViewCell *)[tableView cellForRowAtIndexPath:indexPath]).overlayView.alpha = 1.0;
			
		} completion:^(BOOL finished) {
			((SNTwitterFriendViewCell *)[tableView cellForRowAtIndexPath:indexPath]).overlayView.alpha = 0.0;
		}];
	...
	if ([request isEqual:_idsRequest]) {
		...
		if (error != nil)
			...
			if([[parsedResult objectForKey:@"result"] isEqualToString:@"true"])
				[self.navigationController pushViewController:[[SNFriendProfileViewController alloc] initWithTwitterUser:_vo] animated:YES];

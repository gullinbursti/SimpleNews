...
@interface SNSplashViewController() <MBLResourceObserverProtocol>
	...
	if (_topicIndex == [_topicNames count] - 1) {
		...
		if (_hasDeviceToken && _isIntroComplete) {
			if (FBSession.activeSession.state == FBSessionStateCreatedTokenLoaded) {
				[SNAppDelegate openSession];
				[self.navigationController pushViewController:[[SNRootViewController alloc] init] animated:NO];
			
			} else
				[[self.navigationController topViewController] presentModalViewController:[[SNLoginViewController alloc] init] animated:YES];
...
-(void)_loginViewDismissed:(NSNotification *)notification {
	[self.navigationController pushViewController:[[SNRootViewController alloc] init] animated:YES];
}

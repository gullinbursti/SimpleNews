...
static void filterGreyscale(UInt8 *pixelBuf, UInt32 offset, void *context)
{	
	int r = offset;
	int g = offset+1;
	int b = offset+2;
...
static void filterSepia(UInt8 *pixelBuf, UInt32 offset, void *context)
{	
	int r = offset;
	int g = offset+1;
	int b = offset+2;
...
static void filterPosterize(UInt8 *pixelBuf, UInt32 offset, void *context)
{
...
static void filterSaturate(UInt8 *pixelBuf, UInt32 offset, void *context)
{
...
static void filterBrightness(UInt8 *pixelBuf, UInt32 offset, void *context)
{
	double t = *((double*)context);
...
static void filterGamma(UInt8 *pixelBuf, UInt32 offset, void *context)
{	
...
static void filterOpacity(UInt8 *pixelBuf, UInt32 offset, void *context)
{
...
static double calcContrast(double f, double c){
	return (f-0.5) * c + 0.5;
}

static void filterContrast(UInt8 *pixelBuf, UInt32 offset, void *context)
{
	double val = *((double*)context);
	int r = offset;
	int g = offset+1;
...
static double calcBias(double f, double bi){
	return (double) (f / ((1.0 / bi - 1.9) * (0.9 - f) + 1));
}

static void filterBias(UInt8 *pixelBuf, UInt32 offset, void *context)
{
	double val = *((double*)context);
	int r = offset;
	int g = offset+1;
...
static void filterInvert(UInt8 *pixelBuf, UInt32 offset, void *context)
{	
	int r = offset;
	int g = offset+1;
	int b = offset+2;
...
static void filterNoise(UInt8 *pixelBuf, UInt32 offset, void *context)
{	
...
#pragma mark C Implementation
static double calcOverlay(float b, float t) {
	return (b > 128.0f) ? 255.0f - 2.0f * (255.0f - t) * (255.0f - b) / 255.0f: (b * t * 2.0f) / 255.0f;
}

static void filterOverlay(UInt8 *pixelBuf, UInt8 *pixedBlendBuf, UInt32 offset, void *context)
{	
	int r = offset;
	int g = offset+1;
	int b = offset+2;
...
static void filterMask(UInt8 *pixelBuf, UInt8 *pixedBlendBuf, UInt32 offset, void *context)
{	
	int r = offset;
//	int g = offset+1;
//	int b = offset+2;
...
static void filterMerge(UInt8 *pixelBuf, UInt8 *pixedBlendBuf, UInt32 offset, void *context)
{	
	int r = offset;
	int g = offset+1;
	int b = offset+2;
...
static int calcLevelColor(int color, int black, int mid, int white)
{
	if (color < black) {
		return 0;
	} else if (color < mid) {
...
static void filterLevels(UInt8 *pixelBuf, UInt32 offset, void *context)
{
...
static double valueGivenCurve(CurveEquation equation, double xValue)
{
	assert(xValue <= 255);
	assert(xValue >= 0);
...
static void filterCurve(UInt8 *pixelBuf, UInt32 offset, void *context)
{
...
static void filterAdjust(UInt8 *pixelBuf, UInt32 offset, void *context)
{

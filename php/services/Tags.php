<?php
	...
	class Tags {
		...
		function getActiveTags() {
			...
			$query = 'SELECT * FROM `tblTags` WHERE `active` = "Y";';
			$tag_result = mysql_query($query); 
            
			$tot = 0;
			while ($tag_row = mysql_fetch_array($tag_result, MYSQL_BOTH)) {
				$query = 'SELECT * FROM `tblArticlesTags` WHERE `tag_id` = "'. $tag_row['id'] .'";';
				$article_tot = mysql_num_rows(mysql_query($query));
				
				array_push($tag_arr, array(
					"tag_id" => $tag_row['id'], 
					"title" => $tag_row['title'],
					"info" => $tag_row['info'], 
					"article_total" => $article_tot
				));
				
				$tot++;
	    	}
			
			$this->sendResponse(200, json_encode($tag_arr));
			return (true);
		}
